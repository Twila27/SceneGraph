					...
					int       nCmdShow )
{
	...
	while( uMsg.message != WM_QUIT )
	{
		...
		else
		{
			...
			g_theApp->Update(); //validates WasKeyJustPressed
			render();
		}
	}
							 ...
							 LPARAM lParam )
{
	...
	switch( msg )
	{
		case WM_KEYDOWN:
		{
			...
			else g_theApp->SetKeyDownStatus( wParam, true );
		}
		break;

		case WM_KEYUP:
		{
			g_theApp->SetKeyDownStatus( wParam, false );
		}
		break;
		...
		case WM_SIZE:
		{
			...
			glLoadIdentity();
			gluPerspective( g_perspective_fieldOfView_yDegrees, (GLdouble)nWidth / (GLdouble)nHeight, g_perspective_zNear, g_perspective_zFar);
		}
		break;
...
void init( void )
{
	g_theApp = new TheApp( g_screenWidth, g_screenHeight );
	...
	glMatrixMode( GL_PROJECTION );
	glLoadIdentity();
	g_perspective_aspect = (double)g_screenWidth / (double)g_screenHeight;
	gluPerspective( g_perspective_fieldOfView_yDegrees, g_perspective_aspect, g_perspective_zNear, g_perspective_zFar );

	//Allow transparency use.
	glEnable( GL_BLEND );
	glBlendFunc( GL_SRC_ALPHA, GL_ONE_MINUS_SRC_ALPHA );
	
	glDisable( GL_CULL_FACE );

	//Enable viewing of std::cout, etc.
	AllocConsole();
...
void render( void )
{
	//should really be in a separate update() call, must come before render() body
	g_theApp->Update(); //for keyboard input
