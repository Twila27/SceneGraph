...
public:
	...
	enum VertexGroupingRule { AS_LINES, AS_POINTS, AS_LINE_LOOP, AS_QUADS };
	...
	void DrawSphere( const int vertexGroupingRule, const Vector3& centerPos, float radius, float numSlices, float numSidesPerSlice, float lineThickness = 1.0f, const Rgba& tint = Rgba( ) ) const;
	void DrawCylinder( const int vertexGroupingRule, const Vector3& centerPos, float radius, float height, float numSlices, float numSidesPerSlice, float lineThickness = 1.0f, const Rgba& tint = Rgba( ) ) const;

	void DrawText2D( const Vector2& startBottomLeft, const std::string& asciiText, float cellHeight, const Rgba& tint = Rgba(), const BitmapFont* font = nullptr, float cellAspect = 1.f );
	...
	void DrawVertexArray( const int vertexGroupingRule, const std::vector< Vertex3D_PCT >& vertexArrayData, unsigned int vertexArraySize ) const;
	void DrawVertexArray( const int vertexGroupingRule, const Vertex3D_PCT* vertexArrayData, unsigned int vertexArraySize ) const;

	unsigned int CreateVbo(); //Returns ID.
